...
C4Context
    ...
    Enterprise_Boundary(b0, "Enterprise boundary") {
        ...
        UpdateRelStyle(SystemESB, SystemC, $textColor="red", $lineColor="red", $offsetX="-60", $offsetY="25")
...
C4Container
    ...
    Rel(SystemA, Broker, "Writes messages to broker", "async, AMQP")
    Rel(SystemB, Broker, "Reads messages from broker", "async, AMQP")
    Rel(BackendApi, Broker, "Reads/writes messages for routing other system", "async, HTTPS")
    Rel(SystemB, BackendApi, "Uses for routing other system", "sync, JSON/HTTPS")
    Rel(BackendApi, SystemC, "Proxing C system's HTTP-service and routing calls to it", "sync, JSON/HTTPS, XML/SOAP, etc.")

    UpdateRelStyle(UserESB, WebApp, $textColor="red", $lineColor="red", $offsetX="-90", $offsetY="-150")
    ...
    UpdateRelStyle(SystemA, Broker, $textColor="red", $lineColor="red", $offsetX="-170", $offsetY="-100")
    UpdateRelStyle(SystemB, Broker, $textColor="red", $lineColor="red", $offsetX="-30", $offsetY="-20")
